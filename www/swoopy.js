...
L$1.SwoopyArrow = L$1.Layer.extend({
  ...
  _createArrow: function _createArrow() {
    ...
    if (!this._hideArrowHead) {
      ...
      path.setAttribute('points', '-1.75,-0.75 0,0 -1.75,0.75');
  ...
  _createPath: function _createPath() {
    ...
    var controlLatlng = this._getControlPoint(L$1.latLng(this._fromLatlng), L$1.latLng(this._toLatlng), 0.5);
    var pathOne = L$1.curve(['M', [this._fromLatlng.lat, this._fromLatlng.lng], 'S', [controlLatlng.lat, controlLatlng.lng], [this._toLatlng.lat, this._toLatlng.lng]], {
      animate: false,
      color: this._color,
      fill: false,
  ...
  _getControlPoint: function _getControlPoint(start, end, factor) {
    ...
    var offset = 0.5 * distance - distance;
